            ...
            if (isset($_POST['submit'])) {
                ...
                if (mysqli_fetch_assoc($sql)){
                    if(move_uploaded_file($temp, 'gambar/all_song/' .$gambar_baru)){
                        $query = mysqli_query($conn, "INSERT INTO `lagu` VALUES (NULL, '$id' ,'$title','$genre','$gambar_baru, '$waktu')");
                        $sql2 = mysqli_query($conn, "SELECT id_lagu FROM `lagu` WHERE judul = '$title'");
                        list($id_lagu) = mysqli_fetch_array($sql2);
                        $query2 = mysqli_query($conn, "INSERT INTO lagu_playlist VALUES (NULL, '$playlist','$id_lagu')");
                        if($query2){
                    ...
                    $query_1 = mysqli_query($conn, "INSERT INTO artist VALUES (NULL,'$artist2')");
                    }
                    if(move_uploaded_file($temp, 'gambar/all_song/' .$gambar_baru) ){
                        $sql1 = mysqli_query($conn, "SELECT id FROM artist WHERE nama = '$artist2'");
                        list($id_baru) = mysqli_fetch_array($sql1);
                        $query = mysqli_query($conn, "INSERT INTO `lagu` VALUES (NULL, '$id_baru' ,'$title','$genre','$gambar_baru', '$waktu')");
                        $sql2 = mysqli_query($conn, "SELECT id_lagu FROM `lagu` WHERE judul = '$title'");
                        list($id_lagu) = mysqli_fetch_array($sql2);
                        $query2 = mysqli_query($conn, "INSERT INTO lagu_playlist VALUES (NULL, '$playlist','$id_lagu')");
                        if($query2){
